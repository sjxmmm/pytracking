...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, feature_extractori, classifier, bb_regressor, classification_layer, bb_regressor_layer):
        super().__init__()

        self.feature_extractor = feature_extractor
        self.feature_extractori = feature_extractori
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        train_feati = self.extract_backbone_featuresi(train_imgs[3:,...].view(-1, train_imgs.shape[-3], train_imgs.shape[-2], train_imgs.shape[-1]))
        test_feati = self.extract_backbone_featuresi(test_imgs[3:,...].view(-1, test_imgs.shape[-3], test_imgs.shape[-2], test_imgs.shape[-1]))

        # Classification features
        train_feat_clf = self.get_backbone_clf_feat(train_feat)
        train_feat_clfi = self.get_backbone_clf_feat(train_feati)
        test_feat_clf = self.get_backbone_clf_feat(test_feat)
        test_feat_clfi = self.get_backbone_clf_feat(test_feati)
        # concat 
        train_feat_clf = torch.cat((train_feat_clf, train_feat_clfi), 1)
        test_feat_clf = torch.cat((test_feat_clf, test_feat_clfi), 1)
        ...
        train_feat_iou = [torch.cat((train_feat[l], train_feati[l]), 1).view(num_train_images, num_sequences, train_feat[l].shape[-3]*2, train_feat[l].shape[-2],
                                    train_feat[l].shape[-1]) for l in self.bb_regressor_layer]
        # test_feat_iou = self.get_backbone_bbreg_feat(test_feat)
        test_feat_iou = [torch.cat((test_feat[l], test_feati[l]), 1).view(num_test_images, num_sequences, test_feat[l].shape[-3]*2, test_feat[l].shape[-2],
    ...
    def extract_backbone_featuresi(self, im, layers=None):
        if layers is None:
            layers = self.output_layers
        return self.feature_extractori(im, layers)

    def extract_features(self, im, layers=None):
        if layers is None:
            layers = self.bb_regressor_layer + ['classification']
        if 'classification' not in layers:
            return self.feature_extractor(im, layers), self.feature_extractor(imi, layers)
        backbone_layers = sorted(list(set([l for l in layers + self.classification_layer if l != 'classification'])))
        all_feat = self.feature_extractor(im[:,:3,...], backbone_layers)
        all_feati = self.feature_extractori(im[:,3:,...], backbone_layers)
        feat_cls = torch.cat((all_feat[self.classification_layer], all_feati[self.classification_layer]), 1)
        
        all_feat['classification'] = self.extract_classification_feat(all_feat)
        all_feati['classification'] = self.extract_classification_feat(feat_cls)
        all_feati['layer2'] = torch.cat((all_feat['layer2'], all_feati['layer2']), 1)
        all_feati['layer3'] = torch.cat((all_feat['layer3'], all_feati['layer3']), 1)

        return OrderedDict({l: all_feati[l] for l in layers})
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    backbone_neti = backbones.resnet50(pretrained=backbone_pretrained, frozen_layers=frozen_backbone_layers)
    ...
    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, feature_extractori=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'])
    return net
