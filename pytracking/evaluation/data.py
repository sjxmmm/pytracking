...
class Sequence:
    ...
    def __init__(self, name, frames, framesi, dataset, ground_truth_rect, ground_truth_recti, ground_truth_seg=None, init_data=None,
                 object_class=None, target_visible=None, object_ids=None, multiobj_mode=False):
        self.name = name
        self.frames = frames
        self.framesi = framesi
        self.dataset = dataset
        self.ground_truth_rect = ground_truth_rect
        self.ground_truth_recti = ground_truth_recti
    ...
    def _construct_init_data(self, init_data):
        if init_data is not None:
            ...
            if self.ground_truth_rect is not None and self.ground_truth_recti is not None:
                if self.multiobj_mode:
                    ...
                    assert isinstance(self.ground_truth_recti, (dict, OrderedDict))
                    init_data[0]['bboxi'] = OrderedDict({obj_idi: list(gti[0,:]) for obj_idi, gti in self.ground_truth_recti.items()})
                else:
                    assert self.object_ids is None or len(self.object_ids) == 1
                    if isinstance(self.ground_truth_rect, (dict, OrderedDict)) and isinstance(self.ground_truth_recti, (dict, OrderedDict)):
                        init_data[0]['bbox'] = list(self.ground_truth_rect[self.object_ids[0]][0, :])
                        init_data[0]['bboxi'] = list(self.ground_truth_recti[self.object_ids[0]][0, :])
                    else:
                        init_data[0]['bbox'] = list(self.ground_truth_rect[0,:])
                        init_data[0]['bboxi'] = list(self.ground_truth_recti[0,:])
