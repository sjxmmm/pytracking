...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, imagei, info: dict) -> dict:
        ...
        imi = numpy_to_torch(imagei)

        # Get target position and size
        state = info['init_bbox']
        statei = info['init_bboxi']
        self.pos = torch.Tensor([state[1] + (state[3] - 1)/2, state[0] + (state[2] - 1)/2])
        self.target_sz = torch.Tensor([state[3], state[2]])
        self.posi = torch.Tensor([statei[1] + (statei[3] - 1)/2, statei[0] + (statei[2] - 1)/2])
        self.target_szi = torch.Tensor([statei[3], statei[2]])
        ...
        self.image_szi = torch.Tensor([imi.shape[2], imi.shape[3]])
        ...
        if self.params.get('use_image_aspect_ratio', False):
            ...
            szi = self.image_szi * sz.prod().sqrt() / self.image_szi.prod().sqrt()
            stride = self.params.get('feature_stride', 32)
            sz = torch.round(sz / stride) * stride
            szi = torch.round(szi / stride) * stride
        self.img_sample_sz = sz
        self.img_sample_szi = szi
        self.img_support_sz = self.img_sample_sz
        self.img_support_szi = self.img_sample_szi

        # Set search area
        search_area = torch.prod(self.target_sz * self.params.search_area_scale).item()
        search_areai = torch.prod(self.target_szi * self.params.search_area_scale).item()
        self.target_scale =  math.sqrt(search_area) / self.img_sample_sz.prod().sqrt()
        self.target_scalei =  math.sqrt(search_areai) / self.img_sample_szi.prod().sqrt()

        # Target size in base scale
        self.base_target_sz = self.target_sz / self.target_scale
        self.base_target_szi = self.target_szi / self.target_scalei
        ...
        self.min_scale_factori = torch.max(10 / self.base_target_szi)
        self.max_scale_factori = torch.min(self.image_szi / self.base_target_szi)

        # Extract and transform sample
        init_backbone_feat = self.generate_init_samples(im)
        init_backbone_feati = self.generate_init_samplesi(imi)

        init_backbone_feat = torch.cat((init_backbone_feat, init_backbone_feati),1)
    ...
    def track(self, image, imagei, info: dict = None) -> dict:
        self.debug_info = {}
        ...
        backbone_feat, sample_coords, im_patches, im_patchesi = self.extract_backbone_features(im, imi, self.get_centered_sample_pos(),
    ...
    def extract_backbone_features(self, im: torch.Tensor, imi: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor):
        ...
        im_patchesi, patch_coords = sample_patch_multiscale(imi, pos, scales, sz,
                                                           mode=self.params.get('border_mode', 'replicate'),
                                                           max_scale_change=self.params.get('patch_max_scale_change', None))
        with torch.no_grad():
            backbone_feat = self.net.extract_backbone(im_patches)
            backbone_feati = self.net.extract_backbone(im_patchesi)
        backbone_feat = torch.cat((backbone_feat,backbone_feati),1)
        return backbone_feat, patch_coords, im_patches, im_patchesi
    ...
    def generate_init_samplesi(self, imi: torch.Tensor) -> TensorList:
        mode = self.params.get('border_mode', 'replicate')
        if mode == 'inside':
        # Get new sample size if forced inside the image
            im_szi = torch.Tensor([imi.shape[2], imi.shape[3]])
            sample_szi = self.target_scalei * self.img_sample_szi
            shrink_factori = (sample_szi.float() / im_szi)
            if mode == 'inside':
                shrink_factori = shrink_factori.max()
            elif mode == 'inside_major':
                shrink_factori = shrink_factori.min()
            shrink_factori.clamp_(min=1, max=self.params.get('patch_max_scale_change', None))
            sample_szi = (sample_szi.float() / shrink_factori)
            self.init_sample_scalei = (sample_szi / self.img_sample_szi).prod().sqrt()
            tli = self.posi - (sample_szi - 1) / 2
            bri = self.posi + sample_szi / 2 + 1
            global_shifti = - ((-tli).clamp(0) - (bri - im_szi).clamp(0)) / self.init_sample_scalei
        else:
            self.init_sample_scalei = self.target_scalei
            global_shifti = torch.zeros(2)

        self.init_sample_posi = self.posi.round()
        # Compute augmentation size
        aug_expansion_factori = self.params.get('augmentation_expansion_factor', None)
        aug_expansion_szi = self.img_sample_szi.clone()
        aug_output_szi = None
        if aug_expansion_factori is not None and aug_expansion_factori != 1:
            aug_expansion_szi = (self.img_sample_szi * aug_expansion_factori).long()
            aug_expansion_szi += (aug_expansion_szi - self.img_sample_szi.long()) % 2
            aug_expansion_szi = aug_expansion_szi.float()
            aug_output_szi = self.img_sample_szi.long().tolist()

        # Random shift for each sample
        get_rand_shifti = lambda: None
        random_shift_factori = self.params.get('random_shift_factor', 0)
        if random_shift_factori > 0:
            get_rand_shifti = lambda: ((torch.rand(2) - 0.5) * self.img_sample_szi * random_shift_factori + global_shifti).long().tolist()
        # Always put identity transformation first, since it is the unaugmented sample that is always used
        self.transformsi = [augmentation.Identity(aug_output_szi, global_shifti.long().tolist())]

        augs = self.params.augmentation if self.params.get('use_augmentation', True) else {}

        # Add all augmentations
        if 'shift' in augs:
            self.transformsi.extend([augmentation.Translation(shift, aug_output_szi, global_shifti.long().tolist()) for shift in augs['shift']])
        if 'relativeshift' in augs:
            get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_szi/2).long().tolist()
            self.transformsi.extend([augmentation.Translation(get_absolute(shift), aug_output_szi, global_shifti.long().tolist()) for shift in augs['relativeshift']])
        if 'fliplr' in augs and augs['fliplr']:
            self.transformsi.append(augmentation.FlipHorizontal(aug_output_szi, get_rand_shifti()))
        if 'blur' in augs:
            self.transformsi.extend([augmentation.Blur(sigma, aug_output_szi, get_rand_shifti()) for sigma in augs['blur']])
        if 'scale' in augs:
            self.transformsi.extend([augmentation.Scale(scale_factor, aug_output_szi, get_rand_shifti()) for scale_factor in augs['scale']])
        if 'rotate' in augs:
            self.transformsi.extend([augmentation.Rotate(angle, aug_output_szi, get_rand_shifti()) for angle in augs['rotate']])

        self.init_sample_posi = self.posi.round()
        """Perform data augmentation to generate initial training samples."""

        # Extract augmented image patches
        im_patchesi = sample_patch_transformed(imi, self.init_sample_posi, self.init_sample_scalei, aug_expansion_szi, self.transforms)
        # Extract initial backbone features
        with torch.no_grad():
            init_backbone_feati = self.net.extract_backbone(im_patchesi)

        return init_backbone_feati
